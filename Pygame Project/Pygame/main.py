...
BG = pygame.transform.scale(pygame.image.load("Assets/Background/dark_background.png"), (1540, 810))
...
def play():
    ...
    background = pygame.image.load("Assets/Background/pixel_art_floor_board.jpg").convert()

    zombie_img = pygame.image.load(
        "Assets/Character/Normal Zombie One.png").convert_alpha()
    zombie_img = pygame.transform.scale(zombie_img, (64, 64))
    zombie_img_2 = pygame.image.load(
        "Assets/Character/Knight Zombie One.png").convert_alpha()
    zombie_img_2 = pygame.transform.scale(zombie_img_2, (64, 64))
    zombie_img_3 = pygame.image.load(
        "Assets/Character/Gang Gang Zombie One.png").convert_alpha()
    zombie_img_3 = pygame.transform.scale(zombie_img_3, (64, 64))
    zombie_img_4 = pygame.image.load(
        "Assets/Character/Boss One.png").convert_alpha()
    ...
    rifle_purchased = False
    shotgun_purchased = False
    minigun_purchased = False
    line_purchased = False
    ...
    # Define zombie types
    zombie_types = {
        "normal": {"hp": 2 * ten_hp, "speed": 1.5, "color": (100, 255, 100)},
        "knight": {"hp": 3 * ten_hp, "speed": 1.2, "color": (100, 100, 255)},
        "gang": {"hp": 1 * ten_hp, "speed": 10, "color": (255, 100, 100)},
        "boss": {"hp": 20 * ten_hp, "speed": 1.5, "color": (255, 255, 0)},
    }
    ...
    while running:
        ...
        if not game_over and not shop_menu_open:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
                    if now - last_shot_time > shoot_cooldown:
                        ...
                        if distance != 0:
                            dx /= distance
                            dy /= distance
                        bullets.append({"x": player_x + 32, "y": player_y + 32, "dx": dx, "dy": dy})
                        last_shot_time = now
            ...
            if now - last_wave_time > zombie_spawn_cooldown and not spawn_queue:
                ...
                if wave_number + 1 % 10 == 0 and wave_number != 0:
                    ten_hp *= 2
                    ten_damage += 2
                last_wave_time = now
                zombies_per_wave += 1
                ...
                for j, bullet in enumerate(bullets):
                    ...
                    if zombie_rect.colliderect(bullet_rect):
                        ...
                        if zombie["hp"] <= 0:
                            ...
                            coin_drop_count = 20 if zombie["type"] == "boss" else 1
                            for _ in range(coin_drop_count):
                    ...
                    if not zombie["first_hit"]:
                        if zombie["first_contact_time"] == 0:
                            ...
                            if zombie['type'] == 'normal':
                                player_health -= 3 + ten_damage
                            elif zombie['type'] == 'knight':
                                player_health -= 7 + ten_damage
                            elif zombie['type'] == 'gang':
                                player_health -= 7 + ten_damage
                            elif zombie['type'] == 'boss':
                                player_health -= 15 + ten_damage
        ...
        #     "Assets/Character/MC.png").convert_alpha()
        ...
        # Then inside your game loop, replace the drawing part with:
        mouse_x, mouse_y = pygame.mouse.get_pos()
        angle = math.degrees(math.atan2(player_y - mouse_y, mouse_x - player_x))  # Calculate angle
        rotated_image = pygame.transform.rotate(player_image, angle)
        new_rect = rotated_image.get_rect(center=(player_x + 40, player_y + 40))  # Center the rotation
        ...
        if shop_menu_open:
            ...
            screen.blit(menu_text, (WIDTH // 2 - menu_text.get_width() // 2, HEIGHT // 2 - 225))

            rifle_icon = pygame.image.load(
                "Assets/Icons/Rifle_icon.png").convert_alpha()
            rifle_icon_2 = pygame.transform.scale(rifle_icon, (80, 80))
            screen.blit(rifle_icon_2, (300, 225))
            rifle_text = get_font(35).render("Rifle (+1 Damage)", True, "#781A05")
            screen.blit(rifle_text, (400, 235))

            mouse_pos = pygame.mouse.get_pos()

            rifle_button_text = "$50" if not rifle_purchased else "Purchased"

            rifle_button = Button(
                image=None,
                pos=(1150, 250),
                text_input=rifle_button_text,
                font=font,
                base_color=(0, 0, 0),
                hovering_color=(0, 0, 0)
            )
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if rifle_button.checkForInput(mouse_pos):
                        if y_rifle == 0:
                            if player_coins >= 50:
                                player_coins -= 50
                                player_damage += 1
                                ...
                                y_rifle += 1
            rifle_button.update(screen)

            shotgun_icon = pygame.image.load(
                "Assets/Icons/shotgun_icon.png").convert_alpha()
            shotgun_icon_2 = pygame.transform.scale(shotgun_icon, (80, 80))
            screen.blit(shotgun_icon_2, (300, 320))
            shotgun_text = get_font(35).render("Shotgun (+2 Damage)", True, "#781A05")
            screen.blit(shotgun_text, (400, 320))

            mouse_pos = pygame.mouse.get_pos()

            shotgun_button_text = "$100" if not shotgun_purchased else "Purchased"

            shotgun_button = Button(
                image=None,
                pos=(1150, 330),
                text_input=shotgun_button_text,
                font=font,
                base_color=(0, 0, 0),
                hovering_color=(0, 0, 0)
            )
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if shotgun_button.checkForInput(mouse_pos):
                        if y_shotgun == 0:
                            if player_coins >= 100:
                                player_coins -= 100
                                player_damage += 2
                                shotgun_purchased = True
                                y_shotgun += 1
            shotgun_button.update(screen)

            minigun_icon = pygame.image.load(
                "Assets/Icons/Minigun_icon.png").convert_alpha()
            minigun_icon_2 = pygame.transform.scale(minigun_icon, (80, 80))
            screen.blit(minigun_icon_2, (300,395))
            minigun_text = get_font(35).render("Minigun (+4 Damage)", True, "#781A05")
            screen.blit(minigun_text, (400, 405))

            mouse_pos = pygame.mouse.get_pos()

            minigun_button_text = "$250" if not minigun_purchased else "Purchased"

            minigun_button = Button(
                image=None,
                pos=(1150, 420),
                text_input=minigun_button_text,
                font=font,
                base_color=(0, 0, 0),
                hovering_color=(0, 0, 0)
            )
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if minigun_button.checkForInput(mouse_pos):
                        if y_minigun == 0:
                            if player_coins >= 250:
                                player_coins -= 250
                                player_damage += 4
                                minigun_purchased = True
                                y_minigun += 1
            minigun_button.update(screen)

            line_icon = pygame.image.load(
                "Assets/Icons/yellow_line.png").convert_alpha()
            line_icon_2 = pygame.transform.scale(line_icon, (80, 80))
            screen.blit(line_icon_2, (300, 450))
            line_text = get_font(35).render("Laser Pointer", True, "#781A05")
            screen.blit(line_text, (400, 480))

            mouse_pos = pygame.mouse.get_pos()

            line_button_text = "$75" if not line_purchased else "Purchased"

            line_button = Button(
                image=None,
                pos=(1150, 495),
                text_input=line_button_text,
                font=font,
                base_color=(0, 0, 0),
                hovering_color=(0, 0, 0)
            )
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if line_button.checkForInput(mouse_pos):
                        if y_line == 0:
                            if player_coins >= 75:
                                player_coins -= 75
                                line_purchased = True
                                y_line += 1
            line_button.update(screen)

            shield_icon = pygame.image.load(
                "Assets/Icons/shield_icon.png").convert_alpha()
            shield_icon_2 = pygame.transform.scale(shield_icon, (80, 80))
            screen.blit(shield_icon_2, (300, 510))
            shield_text = get_font(35).render("Shield (+100 hp)", True, "#781A05")
            screen.blit(shield_text, (400, 540))

            mouse_pos = pygame.mouse.get_pos()

            shield_button_text = "$100" if not shield_purchased else "Purchased"

            shield_button = Button(
                image=None,
                pos=(1150, 555),
                text_input=shield_button_text,
                font=font,
                base_color=(0, 0, 0),
                hovering_color=(0, 0, 0)
            )
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if shield_button.checkForInput(mouse_pos):
                        if y_shield == 0:
                            if player_coins >= 100:
                                player_coins -= 100
                                player_health += 100
                                ...
                                y_shield += 1
            shield_button.update(screen)

        if y_line == 1:
            if not game_over:
                mouse_x, mouse_y = pygame.mouse.get_pos()
                pygame.draw.line(screen, (255, 255, 0), (player_x + 32, player_y + 32),
                                 (mouse_x, mouse_y), 4)
...
def main_menu():
    while True:
        ...
        PLAY_BUTTON = Button(image=pygame.image.load("Assets/Buttons/Play Rect.png"), pos=(770, 300),
                             text_input="PLAY", font=get_font(75), base_color="#781A05", hovering_color="White")
        QUIT_BUTTON = Button(image=pygame.image.load("Assets/Buttons/Quit Rect.png"), pos=(770, 600),
